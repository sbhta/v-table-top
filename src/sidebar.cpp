...
SideBar::SideBar(Vector2 pos, Vector2 size, Color bgColor, Level& refLevel):
   ...
   // grid buttons
   addButton(std::make_unique<ToggleButton>(Vector2 {pos.x+60, pos.y+90} , Vector2 {50, 35},
                                            "󰋁 ", "󰋂 ", GFManager.getFont("sidebar-icon", 42), bgColor,BLACK,
                                            false,[this] () {level.switchGrid();}));
   addButton(std::make_unique<TextButton>(Vector2 {pos.x+8, pos.y+130} , Vector2 {30, 35},
                                            "-", GFManager.getFont("sidebar-text", 42), bgColor,BLACK,
                                            [this] () {--level.gridSize;}));
   addButton(std::make_unique<TextButton>(Vector2 {pos.x+118, pos.y+130} , Vector2 {30, 35},
                                            "+", GFManager.getFont("sidebar-text", 42), bgColor,BLACK,
                                            [this] () {++level.gridSize;}));
