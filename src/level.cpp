...
Level::Level() {
   background = {};
   tokens.push_back({"../tokens/TestToken/", {500, 500}});
   tokens.push_back({"../tokens/TestToken/", {500, 500}});
}
Level::~Level(){
   if (background.id > 0){
...
void Level::drawGrid() {
    ...
    for (int x = 0; x < screenWidth; x += gridSize) DrawLine(x, 0, x, screenHeight, gridColor);
    for (int y = 0; y < screenHeight; y += gridSize) DrawLine(0, y, screenWidth, y, gridColor);
